...
#include "../HOYLAND_KAUT_WALLACE/HKW_cubic.h"    // cubic transformation
#include "../HOYLAND_KAUT_WALLACE/misc_macros.h"  // misc. C macros used here (like frand)
#include "../HOYLAND_KAUT_WALLACE/sg_functions.h" // general functions needed for SG
                ...
                int * p_nmbTrial, int * p_nmbIter)
{
	...
	if (Mat_Cholesky(p_TgCorrs, &Chol_TgCorrs, Chol_TestLevel) > 0) {
		...
		printf("\nCHOLESKY DECOMPOSITION OF THE ALLEGED TARGET CORRELATION\n");
	...
	do {
		...
		while (((errMom>MaxErrMom) || (errCorr>MaxErrCorr)) && (iter<MaxIter)) {
			iter++;
			if (TestLevel > 2)
				//printf("\niter %d\n", iter); // original EL 23May2025
				printf("\nIteration %d\n", iter); // modified EL 23May2025
			...
			if (TestLevel > 2) {
				...
				printf("  Overall error in moments: %.4f:\n", errMom);
				if (TestLevel > 3) Mat_Display(&OutMoms, "Moments");
				printf("  Overall error in correlations: %.4f:\n", errCorr);
				if (TestLevel > 3) Mat_Display(&OutCorrs, "Corrs");
			}
			...
			if (TestLevel > 1)
				printf("Overall error in moments and correlations after iteration %2d: %9.6f\n", iter,
				       errMom + errCorr);	   
					   
		} // next iter

		if (TestLevel > 0) {
			printf("\nOverall error in moments and correlations after trial %2d: %9.6f\n", trial, errMom+errCorr);			
