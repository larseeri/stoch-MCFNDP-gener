(Dec 22nd 2025)
## SCENARIOS GENERATOR FOR THE LINEAR, TWO-STAGE, MULTI-COMMODITY, FIXED CHARGE, CAPACITATED NETWORK DESIGN PROBLEM WITH STOCHASTIC RECOURSE<br>
...
modifications.<br>
d- In a Linux terminal, go to the /program folder just created.<br>
...
updated by modifying line 1177 of file main.cpp.) The default file can
...
The configuration file should have this format:<br>

optionKey1   value1<br>
optionKey2   value2<br>
.<br>
.<br>
optionKeyN   valueN<br>
...
this order:<br>
(i) hard-coded initialization values in lines 27-51 of configuration.cpp,<br>
...
taken from default configuration file whose name is specified in line 1177 of
...
The default configuration file is located at ./inout/stoch_config.par. It
has initially been filled with illustrative values and should 
...
this order:<br>
(i) initialization values hard-coded in lines 27-51 of configuration.cpp,<br>
...
prefix as follows:
	...
	docker run --platform linux/amd64 --rm --mount type=bind,src=absolutePathToDockerFolder/inout,dst=/inout \
	stoch_gen_image
...
the host for the Docker instructions to work correctly.<br>
...
Docker's container repository.<br>
...
for randomization of":<br>
1 -> demand volumes<br>
2 -> total capacities of arcs<br>
4 -> commodity-specific capacities of arcs<br>
8 -> fixed costs<br>
16 -> variable costs (either commodity-specific or not)<br>
...
Target moments can be set and saved as follows.<br>
...
options A <double> and B <double>.<br>
...
rejected and options A and B will be ignored.<br>
...
Target correlations can be set and saved as follows.<br>
...
is a shorthand notation for "stands for":<br>
D -> demands<br>
...
the configuration file is ignored.<br>
...
It is interpreted as follows:<br>
-S 3: demands and total capacities of arcs will be randomized<br>
-I instB.std: base deterministic network is to be read from file instB.std<br>
-F G: format of the latter is generic STD<br>
...
pairs and self-correlations are 1.0<br>
-XDA -0.3: target correlations between demands and total capacities of
arcs are -0.3 for all pairs<br>
-XAA 0.7: target correlations among total capacities of arcs are 0.7
for all dissimilar pairs and self-correlations are 1.0<br>
-G: target moments are generated (based on explicit and default
specifications)<br>
-T U: target moments are generated under the assumption that
randomizations each obey a uniform distribution<br>
-A 0.25: parameter alpha defining the latter distribution is 0.25<br>
-B 0.25: parameter beta defining the latter distribution is 0.25<br>
...
volume < 0).<br>
b- Generic STD imposes neither (i), nor (ii).<br>
...
is requested through option S, it is handled through these steps:<br>
...
InstanceGStd::read() in generate.cpp).<br>
...
is formatted as follows (see Section 8 for examples):<br>
a- 1st line shows integer 4 (as there are 4 target moments per randomized
stochastic element and therefore 4 rows in the matrix of target moments),<br>
...
correlations matrix is formatted as follows (see Section 8 for examples):<br>
a- 1st and 2nd lines are identical and show the total number of individual
randomized elements.<br>
...
of the matrix of target moments (cf. Section 4.1.1).<br>
d- Any additional line in the file is ignored.<br>
...
example probs_100_eq_example.dat):<br>
a- 1st line states the number of scenarios<br>
b- 2nd line states all probabilities, separated by spaces<br>
c- Remaining lines of the file are ignored.<br>
This file would be supplied as './probs_100_eq_example.dat' (Linux
instructions) or './inout/probs_100_eq_example.dat' (Docker instructions).<br>
...
convergence fails, the following course of action is recommended:<br>

1- Verify that target moments and correlations are proper.<br>
...
scenarios (see Paragraph 3 below).<br>

2- Verify that slow convergence is not responsible.<br>
...
and 3 above and below.<br>

3- Adjust the number of scenarios.<br>
...
this replaces the default file whose name is hard-coded in line 1120 of
...
are changed according to scenarios are each assumed to follow a
triangular(a, b, c) distribution, where<br>
a = D - (alpha * D),<br>
b = D + (beta * D),<br>
c = D,<br>
D is the base value of the stochastic element taken from the base
